            ...
            public void onClick(View v) {
                ...
                if (cursor != null && cursor.getColumnCount() > 0) {
                    while (cursor.moveToNext()) {
                        ...
                        String dbPhoto = cursor.getString(6);
                        mUser = new User(dbId, dbAccount, dbPassword, dbName, dbSex, dbPhone, dbPhoto);
                    }
                }
                if (mUser == null) {//用户不存在 注册
                    String insertSql = "insert into user(account, password,name,sex, phone,photo) values(?,?,?,?,?,?)";
                    db.execSQL(insertSql, new Object[]{account, password, nickName, sex, phone, imagePath});
